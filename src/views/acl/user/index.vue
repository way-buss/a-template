...
//保存按钮的回调
const save = async () => {
  //点击保存按钮：务必需要保证表单全部符合条件再去发送请求
  await formRef.value.validate()
  //保存按钮：添加新的用户 ｜ 更新已有的用户账户信息
...
const confirmClick = async () => {
  ...
  if (result.code == 200) {
    //提示信息
    ElMessage({ type: 'success', message: '职务分配成功' })
    //关闭抽屉
    drawer1.value = false
    //刷新数据(获取更新完毕用户的信息，更新完毕留在当前页)
...
const deleteSelectUser = async() => {
     ...
     //批量删除的请求
     const result1:any = await reqSelectRemoveUser(idList)
     if(result1.code == 200){
          ElMessage({type:'success',message:'删除成功'})
          getHasUser(userArr.value.length>1?pageNo.value:pageNo.value-1)
     }
